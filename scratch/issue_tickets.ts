...
async function run() {
  ...
  const targetEvent = events.find(e => e.title === "The Things We Do on Tables");
  if (!targetEvent) {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const newTicket = {
      ...
      quantity: 1,
      totalPaid: 5000,
      status: "purchased" as "purchased" | "checked_in",
      tierName: "Regular",
  ...
  // Generate the exact same email HTML
  ...
  console.log("Successfully recorded 6 tickets and queued email logs!");
}
run();
